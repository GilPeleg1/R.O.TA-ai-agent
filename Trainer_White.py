...
wandb.init(
    ...
    project="White_DQN_Training_Against_MinMax",
...
def main ():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        ### log the data, update the best results ###
        if (epoch+1) % 100 == 0:
            print(f'\nres= {res}')
            ...
            if best_res < res:      
                best_res = res
            wandb.log({"AvgLoss": avgLoss, "loss": loss, "wins": res, "steps": step})
            res = 0

        #### run 100 games against the said agent ####
        if (epoch+1) % 100 == 0:
             test = tester(100)
