...
class DQN_Agent:
    ...
    def get_Action (self, state:State, epoch = 0, event= None, train = True, graphics = None, black_state = None) -> tuple:
        actions = self.env.get_all_legal_actions(state)
        if self.train and train:
            epsilon = self.epsilon_greedy(epoch)
            rnd = random.random()
            if rnd < epsilon and actions != None:
        ...
        if self.player == 1:
            ...
            state_tensor = black_state.toTensor()
        # state_tensor = state.toTensor()
        action_tensor = torch.tensor(np.array(actions).reshape(-1,4))
    ...
    def get_Actions (self, states_tensor: State, dones) -> torch.tensor:
        ...
        for i, board in enumerate(states_tensor):
            if dones[i].item():
                ...
                actions.append(self.get_Action(State.tensorToState(state_tensor=board,player=self.player), train=True))
