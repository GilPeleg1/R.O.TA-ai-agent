...
class ReplayBuffer:
    ...
    def push (self, state : State, action, reward, next_state: State, done):
        action_tensor = torch.from_numpy(np.array(action).reshape(-1,4))
        self.buffer.append((state.toTensor(),action_tensor , torch.tensor(reward), next_state.toTensor(), torch.tensor(done)))
